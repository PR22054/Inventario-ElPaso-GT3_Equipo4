...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 01-10-2025 a las 09:02:55
...
INSERT INTO `herramienta` (`herramienta_id`, `herramienta_codigo`, `herramienta_nombre`, `herramienta_descripcion`, `herramienta_precio`, `herramienta_stock`, `herramienta_foto`, `categoria_id`, `usuario_id`, `necesita_mantenimiento`, `estado_alarma`, `requiere_mantenimiento`) VALUES
(1, 'HM001', 'Martillo', 'Herramienta manual de percusión con cabeza metálica y mango ergonómico, ideal para trabajos en general.', 15.00, 20, 'Martillo_50.png', 1, 1, 0, 0, 0),
(3, 'HE001', 'Taladro Eléctrico Bosch', 'Taladro eléctrico de marca Bosch con motor potente, velocidad variable y percutor. Perfecto para perforar madera, metal y concreto en aplicaciones profesionales.', 120.00, 5, 'Taladro_Eléctrico_Bosch_98.png', 2, 1, 1, 1, 1),
(4, 'HE002', 'Sierra Circular', 'Sierra circular eléctrica de alto rendimiento para cortes rectos precisos en materiales derivados. Incluye guía de corte y protección de seguridad.', 250.00, 3, 'Sierra_Circular_52.png', 2, 1, 1, 1, 1),
(5, 'DI001', 'Scanner OBD2 Profesional', 'Escáner automotriz profesional para diagnóstico de vehículos mediante puerto OBD2. Lee y borra códigos de falla, muestra datos en tiempo real y compatible con múltiples protocolos.', 180.00, 4, 'Scanner_OBD2_Profesional_7.png', 8, 1, 1, 1, 1),
(6, 'DI002', 'Multímetro Digital', 'Instrumento de medición digital para verificar voltaje, corriente y resistencia eléctrica. Pantalla LCD de fácil lectura y protección contra sobrecargas.', 45.00, 8, 'Multímetro_Digital_57.png', 8, 1, 0, 0, 0),
(7, 'HE003', 'Gato Hidráulico 3T', 'Gato hidráulico tipo botella con capacidad de 3 toneladas para elevación de vehículos. Sistema de bombeo manual y válvula de seguridad integrada.', 180.00, 2, 'Gato_Hidráulico_3T_1.png', 2, 1, 1, 1, 1),
(8, 'HE004', 'Compresor de Aire 100L', 'Compresor de aire con tanque de 100 litros, motor de alta potencia y regulador de presión. Ideal para taller mecánico, pintura y herramientas neumáticas.', 300.00, 1, 'Compresor_de_Aire_100L_43.png', 2, 1, 1, 1, 1),
(9, 'HE005', 'Elevador Hidráulico 2P', 'Elevador hidráulico de dos postes para vehículos, capacidad profesional. Sistema de seguridad con traba automática y brazos ajustables para diferentes tipos de automóviles.', 800.00, 2, 'Elevador_Hidráulico_2P_1.png', 2, 1, 1, 0, 1);
-- --------------------------------------------------------
...
(1, 'P001', 'Aceite Motor 10W40', 15.00, 50, 'Aceite_Motor_10W40_55.png', 3, 1),
(2, 'P002', 'Filtro de Aceite', 8.00, 30, 'Filtro_de_Aceite_83.png', 4, 1),
(3, 'P003', 'Batería 12V 60Ah', 120.00, 10, 'Batería_12V_60Ah_94.png', 5, 1),
(4, 'P004', 'Neumático 195/65 R15', 75.00, 20, 'Neumático_195_65_R15_51.png', 6, 1),
(5, 'P005', 'Neumático 205/55 R16', 85.00, 15, 'Neumático_205_55_R16_42.png', 6, 1),
(6, 'P006', 'Filtro de Combustible', 12.00, 25, 'Filtro_de_Combustible_31.png', 4, 1),
(7, 'P007', 'Líquido de Frenos DOT4', 8.50, 40, 'Líquido_de_Frenos_DOT4_24.png', 3, 1),
(8, 'P008', 'Guantes de Seguridad', 5.50, 100, 'Guantes_de_Seguridad_89.png', 7, 1),
(9, 'P009', 'Cables de Arranque 400A', 25.00, 10, 'Cables_de_Arranque_400A_68.png', 5, 1),
(10, 'P010', 'Bombilla Halógena H4', 5.00, 50, 'Bombilla_Halógena_H4_41.png', 5, 1),
(11, 'P011', 'Aceite Transmisión 75W90', 18.00, 30, 'Aceite_Transmisión_75W90_8.png', 3, 1);
...
--
ALTER TABLE `salida_producto`
  MODIFY `salida_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=14;

--
-- Filtros para la tabla `salida_producto`
--
ALTER TABLE `salida_producto`
  ADD CONSTRAINT `salida_producto_ibfk_1` FOREIGN KEY (`producto_id`) REFERENCES `producto` (`producto_id`),
  ADD CONSTRAINT `salida_producto_ibfk_2` FOREIGN KEY (`usuario_id`) REFERENCES `usuario` (`usuario_id`);
COMMIT;
